...
class ClientRecord:
  ...
  def get_sequencenum(self,filename):
    ...
    try:
      sequence_num = self.cursor.execute("SELECT sequencenum FROM records WHERE filename='{}';".format(filename)).fetchone()[0]
    except sqlite3.Error, e:
  ...
  def update_sequencenum(self, filename, new_sequencenum=None):
    ...
    try:
      sequencenum = self.cursor.execute("SELECT sequencenum FROM records WHERE filename='{}';".format(filename)).fetchone()[0]
    except sqlite3.Error, e:
    ...
    if sequencenum:
      try:
        ...
        sequencenum = new_sequencenum if new_sequencenum else sequencenum+1
  ...
  def delete_directory_records(self,filename):
    ...
    logging.debug("deleting directory: {}".format(directory_to_delete))
    try:
